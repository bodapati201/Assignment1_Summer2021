...
Note: Use of String function (Contains) and hasmap is not allowed, think of other ways how you could the numbers be represented
        */
        ...
        private static bool CheckIfPangram(string s)
        {
            try
            {
                //The following code loops through all the characters of the provided string and returns true if all the alphabets are found.
 ...
 <returns>int </returns>
     */
        ...
        private static int NumIdenticalPairs(int[] arr)
        {
            try
            {
                //Using nested loops to identify the good pairs. Each number is compared to numbers with higher index, when they match, counter will be incremented.
                int count = 0;
  ...
  Right sum = nums[4] + nums[5] = 5 + 6 = 11
      ...
      */
        private static int PivotIndex(int[] nums)
        {
            try
            {
                //We are using nested for loops; one will sum the numbers from lower to higher index, other loop will sum the numbers from higher to lower indexes. If both are matching, correponding index will be returned.
    ...
    */
        ...
        private static string MergeAlternately(string word1, string word2)
        {
            try
            {
                //Input words are split into character arrays, and then each character is appended into StringBuilder one after the other.
                int temp = 0;
